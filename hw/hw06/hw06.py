...
class VendingMachine:
    ...
    # Fill in the VendingMachine class, adding attributes and methods as appropriate, such 
    # that its behavior matches the following doctests:
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        n = Mint.current_year - self.year
        if n > 50:
            return self.cents + (n - 50)
        return self.cents
...
def is_bst(t):
    ...
    def bst_min(t):
        ...
        if t.is_leaf():
            ...
            return min([bst_max(b) for b in t.branches] + [t.label])
    ...
    if t.is_leaf():
        ...
        if t.label >= bst_max(left) or t.label <= bst_min(right):
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def digit_helper(n, lst):
        if n == 0:
            return lst
        else:
            lst = Link(n % 10, lst)
            n = n // 10
            return digit_helper(n, lst)
